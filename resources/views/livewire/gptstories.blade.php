    ...
    <div class="mt-4">
            ...
            <div class="w-full my-2">
                <div class="w-full relative">
                    <label class="relative">
                        <input class="h-10 w-full" wire:keydown="updateSearch($event.target.value)" type="text" placeholder="Search for stories">
                        <span class="absolute top-[-10px] left-[15px] text-[8px] text-gray-300">Powered by Raccoon</span>
                    </label>
        ...
        @foreach($stories as $story)
            <div class="story-card border-b-2">
                ...
                @php
                // 40 words
                    $story['content'] = implode(' ', array_slice(explode(' ', $story['content']), 0, 40));
                @endphp
                ...
                <div class="flex justify-end gap-2 mb-3">
                    ...
                    <div class="flex tooltip gap-3 mb-3">
                        <a href="{{ route('gptstories.show', $story) }}" class="btn ring ring-inset">
                            View Story
                        </a>
                    </div>
                    ...
                    @if(auth()->user() && Gate::allows('manage'))
                        ...
                        <div data-tip="Edit Story" class="flex tooltip gap-3 mb-3">
                            <a href="{{ route('gptstories.edit', $story) }}" class="btn ring ring-inset">
                                <i class="fa-solid fa-pen-nib"></i>
                            </a>
                        </div>

                        <form data-tip="Delete Story" action="{{ route('gptstories.destroy', $story) }}" method="POST" class="d-inline tooltip">
                            @csrf
                            @method('DELETE')
                ...
                <section class="flex text-xs gap-4 mb-3">
                    ...
                    <div class="flex align-middle items-center flex-wrap gap-1 mb-2">
                        @foreach ($story->scriptures as $scripture)
                            ...
                            @endphp
                            <i style="color: {{$randomColor}}" class="fa-solid fa-bookmark"></i> <span class="tag mr-3">{{ $scripture->verse }}</span>
                        @endforeach
                    </div>
